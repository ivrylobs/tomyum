...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $videos =  Blog::select(['id','title','slug','excerpt','user_id','published_at','likes','dislikes'])
        ...
        return view('index', compact('banners', 'sliders', 'trending', 'weekly_popular', 'recent', 'additional_items', 'videos', 'recommendations'));
    }
    ...
    public function browseCategorySubGrp(Request $request, $slug, $sortby = Null)
    {
        ...
        return view('category_sub_group', compact('categorySubGroup', 'products', 'brands', 'priceRange', 'sliders'));
    }
    ...
    public function browseCategoryGroup(Request $request, $slug, $sortby = Null)
    {
        $categoryGroup = CategoryGroup::where('slug', $slug)->with(
            ...
            }])->orderby('id', 'desc')->active()->firstOrFail();
        ...
        $trending = Inventory::orderby('view_count', 'desc')->take(6)->get();
    ...
    public function shop($slug)
    {
        ...
        $featuredProducts = Inventory::where('shop_id', $shop->id)
            ->groupBy('product_id')
            ->filter(request()->all())
            ->with(['feedbacks:rating,feedbackable_id,feedbackable_type', 'images:path,imageable_id,imageable_type'])
            ->whereHas('tags', function($q) {
        ...
        return view('shop', compact('shop', 'products', 'featuredProducts'));
    }
    ...
    private function update_recently_viewed_items($item)
    {
        ...
        $inventory->min_order_quantity = 10;
        $inventory->save(); // Commented this line to prevent error in sql query
