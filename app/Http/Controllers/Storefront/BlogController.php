...
class BlogController extends Controller
{
    ...
    public function index()
    {
        ...
        ->paginate(config('system.view_blog_post_per_page', 4), ['*'], 'blog');

        $vdo = Blog::select(['id','title','slug','excerpt','user_id','published_at','likes','dislikes'])
            ->published()->recent()->with('author:id,name,nice_name','tags')->withCount('comments')
            ->whereHas('tags', function($q) {
                $q->where('tags.id', 31);
            })
            ->paginate(config('system.view_blog_post_per_page', 4), ['*'], 'vdo');

        $tags = $this->getTags($blogs);

        return view('blog', compact('blogs','tags', 'vdo'));
    }
    ...
    {
        // $blog = Blog::where('slug', $slug)->published()
        // ->with(['comments' => function($q){
        //     $q->published()->with('author:id,name,nice_name');
        // }])->firstOrFail();

        $blog = Blog::where('slug', $slug)->published()->firstOrFail();
        $vdo = Blog::where('slug', $slug)->published()->firstOrFail();

        return view('blog', compact('blog', 'vdo'));
    }
