...
class ListHelper
{
    ...
    public static function categoriesForTheme($all = False)
    {
        ...
        ->with(['images:path,imageable_id,imageable_type', 'subGroups' => function($query) use ($all){
           ...
           // Uncommented this line to fix CatSlider 
    ...
    {
//        Blog::select(['id','title','slug','excerpt','user_id','published_at','likes','dislikes'])
...
//
        $blog = Blog::select(['id','title','slug','excerpt','published_at'])
            ->whereHas('tags', function($q) {
                $q->where('tags.id', 32);
            })
            ->published()->recent()->take(3)->get();

        return $blog;
    }

    public static function recentVdo()
    {
        $blog = Blog::select(['id','title','slug','excerpt','published_at'])
            ->whereHas('tags', function($q) {
                $q->where('tags.id', 31);
